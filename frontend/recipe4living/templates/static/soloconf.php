	<div id="main-content" class="static">
		<table border="0">
			<tr>
				<td width="600px" valign="top">
					<h1>Confirmation</h1>
					<p>
					<b>You</b> have successfully been unsubscribed from <b>Recipe4Living Special Offers</b>.
					</p><br><br>
					<p>
					Thank you for updating your preferences. Please allow up to 3 days for your changes to go into effect.
					</p>
					<br><br>
					<p>If you have unsubscribed in error - <a href="http://www.dailygiftsforinput.com/dispatch2.asp?home=70-28629O-L4" target="_blank">please click here to resubscribe</a>.</p>
				</td>
				<td width="300px" valign="top"><div id="panel-right" class="column">
				<div class="ad"><?php $this->_advert('openx_300x250atf'); ?></div>
				<?php $this->_box('reference_guides', array('limit' => 10)); ?>
				<div class="ad"><?php $this->_advert('WEBSITE_RIGHT_BANNER_1'); ?></div>
			</div></td>
			</tr>
		</table>
	</div>
