...
*/
	...
	_preload: function(fast){
		...
		if (this.preloader.retrieve('loaded') /*&& this.preloader.offsetWidth*/ && $time() > this.delay && $time() > this.transition){
			...
			this.image.set('styles', {'display': 'block', 'height': 'auto', 'visibility': 'visible', 'width': 'auto', 'zIndex': this.counter});
...
*/
	...
	_thumbnails: function(){
 		if (this.options.thumbnails !== false) 
		...
		this.data.thumbnails.each(function(thumbnail, i){
			...
			if (this.options.thumbnailType == 'tab') {
				a.set('text', this.data.captions[i]);
			} else {
	 ...
	 *	@return Slideshow
	 */
	attachControl: function(event, action, element) {
		$(element).addEvent(event || 'click', function(event, action) {
			if (event) {
				event.stop();
			}
			switch (action) {
		...
		}.bindWithEvent(this, action));
		return this;
	},
	
	/**
	 *	Attach multiple controls
	 *
	 *	@access public
	 *	@param string Event name
	 *	@param object
	 *	@return Slideshow
	 */
	attachControls: function(event, elements) {
		$H(elements).each(function(element, action) {
			this.attachControl(event, action, element);
		}, this);
		return this;
	}
