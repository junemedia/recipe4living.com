...
var Ratings = new Class({
	...
	initialize: function(element, options) {
		...
		/* Set up */
		this.setOptions(options);
		this.stars = $(element).getElements(this.options.selector);
	...
	_setNormal: function(index) {
		...
		if (this.stars.length < index){
			return false;
		}
		
		var star = this.stars[index].getElement('img');
		star.set('src', SITEASSETURL + '/images' + this.options.offSrc);
	...
	_setFilled: function(index) {
		...
		var star = this.stars[index].getElement('img');
		star.set('src', SITEASSETURL +'/images' + this.options.onSrc);
...
var FontResizer = new Class ({
	...
	initialize: function(element, options) {
		...
		// Get initial size
		this._resize(this.options.initial);
	},
	
	/**
	 *	Test if potential size is out of bounds
	 *
	 *	@param float Size
	 *	@return bool Out of bounds
	 */
	_outOfBounds: function(size) {
		return ($chk(this.options.min) && (size < this.options.min)) || ($chk(this.options.max) && (size > this.options.max));
	},
	 ...
	 */
	increase: function() {
		return this._resize(this.size + this.options.step);
	},
	 ...
	 */
	reset: function() {
		return this._resize(this.options.initial);
	},
	
	/**
	 *	Change the element's text size
	 *
	 *	@param float Potential font size
	 *	@return bool Changed
	 */
	_resize: function(fontSize) {
		
		// Out of bounds
		if (this._outOfBounds(fontSize)) {
			return false;
		}
		
		// Set size
		this.size = fontSize;
		return this.element.setStyle('font-size', this.size+'em');
	}
});
