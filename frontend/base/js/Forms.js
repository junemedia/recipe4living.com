...
var FancyForm = new Class({
	...
	onInputClick: function(event, input, el) {
		...
		if (input.getProperty('disabled')) { return; }
		//if (!el.hasClass(this.options.onclasses[el.type])) { this.select(el); }		//changes: 1. change "onclasses" to "offclasses", 2. remove "!"
		if (el.hasClass(this.options.offclasses[el.type])) {this.select(el); }
...
/**
 *	A simpler version of Overtext, without the positioning issues.
 *
 *	@package BluApplication
 *	@subpackage FrontendJS
 */
var InputText = new Class({
	...
	initialize: function(element, options) {
		this.input = $(element);
		this.text = this.options.textOverride || this.input.get('alt') || this.input.get('title') || this.text;
		this.setOptions(options);
		...
		this.input.getParent('form').addEvent('submit', this.onFormSubmit.bindWithEvent(this));
		
		// If no existing value, set initial status
	...
	/**
	 *	What happens when input is focused.
	 *
	 *	@param event
	 */
	onFocus: function(event) {
		if ((this.input.get('value') == this.text) && !this.input.get('disabled')) {
			this._clear();
		}
	},
	
	/**
	 *	What happens when input is blurred.
	 *
	 *	@param event
	 */
	onBlur: function(event) {
		if (!this.input.get('value') && !this.input.get('disabled')) {
			this._reset();
		}
	},
	 ...
	 */
	_reset: function() {
		this.input.addClass('default').set('value', this.text);
	},
	 ...
	 */
	_clear: function() {
		this.input.removeClass('default').set('value', '');
	},
	
	/**
	 *	What happen when the containing form is submitted (validation?)
	 *
	 *	@param event
	 */
...
var StandardForm = new Class({
	...
	buildContent: function() {
		...
		/* Get action URL */
		this.url = action ? action.split('#', 1) : [action];
		this.url = this.url[0];
		
		/* Update fallback action to accomodate page scrolls on error */
		this.form.set('action', action ? action.replace('#', '#scroll-') : action);
		...
		this.form.getElements('button.reloads, input.reloads, a.reloads').each(function(button) {
			button.addEvent('click', this.onReloadButtonClick.bindWithEvent(this, button));
		}, this);
		this.form.getElements('label.reloads').each(function(label) {
			label.addEvent('click', this.onReloadLabelClick.bindWithEvent(this, label));
		}, this);
	...
	onReloadLabelClick: function(event, label) {
