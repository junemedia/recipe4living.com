	...
	if (!eregi("^[A-Za-z0-9\._-]+[@]{1,1}[A-Za-z0-9-]+[\.]{1}[A-Za-z0-9\.-]+[A-Za-z]$", $email)) {
		...
		if (!getmxrr($domain, $mxhosts)) {
			...
			if ($error == '') {
				...
				}elseif(BullseyeBriteVerifyCheck($email) == false) {
                    $error = 'Invalid Email Address(3)';
                    $email_check_passed = false;
...
<style type="text/css">
#divBG {
background-image:url('http://pics.recipe4living.com/squeeze/Copycat-Top-Secret-Female-Chef.png');
...
<div id="divBG">
	...
	<div id="emailRow">
		<input style="background-color: #fdee8f;" type="text" name="email" value="<?php echo $email; ?>" size="25" maxlength="100"><br><br>
